 ...
 */
class PacketReader {
	private static final String LOGTAG = "PACKET_READER";
...
/*    public void startKeepHeartBeat(XmppManager xmppManager){
        ...
        try {
            ...
            do {
                if (eventType == XmlPullParser.START_TAG) {
                	Log.e(LOGTAG, "eventType == XmlPullParser.START_TAG");
                    if (parser.getName().equals("message")) {
                    	Log.e(LOGTAG, "getname==message");
                        processPacket(PacketParserUtils.parseMessage(parser));
                    }
                    else if (parser.getName().equals("iq")) {
                    	Log.e(LOGTAG, "getname==iq");
                        processPacket(PacketParserUtils.parseIQ(parser, connection));
                    }
                    else if (parser.getName().equals("presence")) {
                    	Log.e(LOGTAG, "getname==presence");
                        processPacket(PacketParserUtils.parsePresence(parser));
                    }
                    // We found an opening stream. Record information about it, then notify
                    // the connectionID lock so that the packet reader startup can finish.
                    else if (parser.getName().equals("stream")) {
                    	Log.e(LOGTAG, "getname==stream");
                    	...
                    	Log.e(LOGTAG, "getname==challenge");
                    	...
                    	Log.e(LOGTAG, "getname==compressed");
                	...
                	Log.e(LOGTAG, "eventType == XmlPullParser.END_TAG");
     ...
     * 
     * 处理一个包当它已经被充分解析过后，解析是通过已安装packet收集器和监听器，然后让它们检验packet,
     * 看一下他们是否有一个匹配的过滤器来匹配它
     * 
     */
    private void processPacket(Packet packet) {
    	 Log.d(LOGTAG, "packet = " + packet.toXML());
        if (packet == null) {
            return;
        }
        ...
        Log.d(LOGTAG, "in the packetReader, packet ="  + packet.toXML());
    ...
    private class ListenerNotification implements Runnable {
        ...
        public void run() {
        	Log.i("ListenerNotification", "begin to notify the listener");
