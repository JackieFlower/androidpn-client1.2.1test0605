...
public class PacketParserUtils {
    ...
    public static IQ parseIQ(XmlPullParser parser, Connection connection) throws Exception {
        ...
        Log.d("PacketParserUtils", "id ="+id);
        Log.d("PacketParserUtils", "to ="+to);
        Log.d("PacketParserUtils", "from ="+from);
        Log.d("PacketParserUtils", "type ="+type);
        ...
        while (!done) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                String elementName = parser.getName();
                Log.d("PacketParserUtils", "elementName ="+elementName);
                
                String namespace = parser.getNamespace();
                Log.d("PacketParserUtils", "namespace ="+namespace);
                ...
                // this element name and namespace.
                	...
                	Log.d("PacketParserUtils", "begin to the provider");
                    Object provider = ProviderManager.getInstance().getIQProvider(elementName, namespace);
                   
                    Log.d("PacketParserUtils", "the provider is " + provider.toString());
                    
                    Log.d("PacketParserUtils", "the provider == null ? " + (provider == null));
                    
                    if (provider != null) {
                        if (provider instanceof IQProvider) {
                            iqPacket = ((IQProvider)provider).parseIQ(parser);
                            
                            Log.d("PacketParserUtils", provider.toString() + 
                            		"provider instanceof NotificationIQProvider" + 
                            		(provider instanceof NotificationIQProvider));
                            Log.d("PacketParserUtils", "get into the provider");
                        }
                        else if (provider instanceof Class) {
                        	Log.d("PacketParserUtils", "get into the class instead of the provider");
    ...
    private static Authentication parseAuthentication(XmlPullParser parser) throws Exception {
        ...
        boolean done = false;
        while (!done) {
 //       	Log.e("PacketParserUtils",	 parser.toString());
            int eventType = parser.next();
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals("username")) {
                	String aUsername = parser.nextText();
                	Log.e("PacketParserUtils", aUsername);
                    authentication.setUsername(aUsername);
                }
